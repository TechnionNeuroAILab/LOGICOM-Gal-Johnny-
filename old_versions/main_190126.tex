...
Semantic recall is initiated whenever a word is recalled during episodic traversal. \omri{It is also based on a depth-first search, but the underlying structure is a semantic graph (and not a tree, as in the episodic setting).}
\begin{enumerate}
...
We begin with a mathematical definition of \emph{Free Recall Test}.
...
\label{thm:main-log}
Consider a free-recall experiment with \(k\) studied words and an TOM model with depth \(d\) and chunk size \(C\). There exist constants \(a,b>0\) such that, defining
\[
I \;=\;
\left[
...
Consider a free-recall experiment with \(k\) studied words and an TOM model with depth \(d\) and chunk size \(C\). For every $0<\alpha <1$, there exist constants \(a,b>0\) such that, defining
\[
I =
\left[
...
Toward the proof, we assume that the semantic component can be modeled as a simple graph in which every existing edge is traversed with probability~1. By well-known results in random graph theory \parencite{Bollobás_2001}, with high probability, the largest connected component of this graph has size \(\Theta(\log k)\). Both semantic models proposed in this work can be reduced to this setting, although we do not present the reduction here.
...
Given a free recall test of \(k\) words and a simple-TOM model with depth \(d\), chunk size \(C\), a semantic component modeled by a simple graph (having 1 traversal probability) with maximum CCP of size \(\Theta(\log(k))\) w.h.p, and tree-edge traversal probability \(p\) (with \(0<p<1\)). Then w.h.p there exists an interval \(I \subset [0,1]\) such that for all \(p \in I\):
\[
...
Fix the semantic graph and let \( C_1,\dots,C_m \) denote its connected components, where \( m \) is the number of connected components. Define the random variables
\[
X_i =
\begin{cases}
...
Choose constants \(a,b > 0\) such that \( b > a \log(k) \), and define the interval
...
\]
for the power-law regime \parencite{naim2020fundamental}, and
\[
